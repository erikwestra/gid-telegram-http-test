...
// Import dependencies
var crypto = require('crypto-js'); // Updated EJW.
var randomBytes = require('react-native-randombytes'); // Added EJW.
var BigInteger = require('jsbn').BigInteger; // FIXED BY EJW.
var getLogger = require('get-log');
var utility = require("../../tl").utility;
...
function createSHAHash(buffer, algorithm) {
  try {
    ...
    //var sha1sum = crypto.createHash(algorithm || 'sha1'); -- Replaced by EJW.
    var sha1sum = crypto.algo.SHA1.create();
    if (require('util').isArray(buffer)) {
        if (logger.isDebugEnabled()) {
            logger.debug('It\'s an Array of buffers');
        }
        for (var i = 0; i < buffer.length; i++) {
            // sha1sum.update(buffer[i]); -- Replaced by EJW.
            var s = buffer[i].toString("hex");
            var wordArray = crypto.enc.Hex.parse(s);
            sha1sum.update(wordArray);
        }
    } else {
        if (logger.isDebugEnabled()) {
            logger.debug('It\'s only one buffer');
        }
        // sha1sum.update(buffer); -- Replaced by EJW.
        var s = buffer.toString("hex");
        var wordArray = crypto.enc.Hex.parse(s);
        sha1sum.update(wordArray);
    }
    //return sha1sum.digest(); -- Replaced by EJW.
    var digest = sha1sum.finalize().toString(crypto.enc.Hex);
    return new Buffer(digest, "hex");
  } catch (error) {
    logger.debug("EXCEPTION IN createSHAHash");
    logger.debug(error);
...
function createRandomBuffer(bytesLength) {
    //return new Buffer(crypto.randomBytes(bytesLength));  -- replaced EJW.
    return new Buffer(randomBytes.randomBytes(bytesLength));
}
