...
// The code:
function retrieveAuthKey(authKeyBuffer, authKeyPassword, promised) {
    return mt.auth.AuthKey.decryptAuthKey(authKeyBuffer, authKeyPassword, promised);
}

function retrievePlainAuthKey(key) {
    return mt.auth.AuthKey.setPlain(key);
}
...
function Client(app, dataCenter) {
    ...
    if (app.authKey && app.serverSalt) {
      this._channel = createEncryptedChannel(
        this._connection,
        app,
        app.authKey,
        app.serverSalt
...
require('util').inherits(Client, require('eventemitter2').EventEmitter2);
...
module.exports = {
  createClient,
  retrieveAuthKey,
  retrievePlainAuthKey,
  authKeyWithSaltToStorableBuffer,
  restoreAuthKeyWithSaltFromStorableBuffer,
  newAuthKey,
  telegram: staticInfo.telegram,
